...
- 当某个号码历史已经接过验证码时，项目会在**本次页面触发发送短信之前**，先调用 Hero-SMS 的“等待新短信”状态切换（等价于网页上的“重新发送短信”）
- 如果某个 Hero-SMS 号码历史从未接过码，则从**本次点击发送短信按钮**开始计时；**2 分钟**内仍未收到任何验证码时，会自动取消当前 activation、把该号码标记为本地耗尽，并把当前 attempt 交给外层重试
- 如果某个 Hero-SMS 号码历史已经接过 **1~2 次**验证码，则从**本次点击发送短信按钮**开始计时；**3 分钟**内仍未收到新的验证码时，同样会自动取消当前 activation、把该号码标记为本地耗尽，并把当前 attempt 交给外层重试
