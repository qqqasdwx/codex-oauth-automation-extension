...
- 最近打开的来源地址
- LuckMail 当前运行时选择
- OutlookEmail 当前运行时选择（`currentOutlookEmailAccountId`、`outlookEmailAccounts`）
...
- CPA / SUB2API 配置
- 邮箱 provider 配置
- OutlookEmail Base URL / 登录密码 / 源分组 / 成功分组 / 已拉取分组缓存
- Hotmail 账号池
- 2925 账号池
...
1. 后台先清理 ChatGPT / OpenAI 域下 cookies，尽量以干净状态启动新一轮
2. 关闭当前残留的 ChatGPT / OpenAI 注册登录相关标签页，避免复用脏页面
3. 新开 ChatGPT 官网注册入口标签页
4. 等待注册页内容脚本就绪（仅做连接与注入确认）
5. 不在 Step 1 检测或点击注册入口
6. 标记 Step 1 完成
...
- 自动重新发送验证码次数现在使用 sidepanel 里的单一“验证码重发”配置；普通邮箱仍按 25 秒间隔节流，Hotmail / 2925 不走这个 25 秒间隔。Step 4 / Step 8 进入验证码页后默认先轮询当前这次发送产生的验证码，不再在页面刚进入时额外预点一次“重新发送”；只有轮询未命中或页面拒绝验证码时，才会消耗自动重发次数。
- 验证码提交流程现在会先让内容脚本立即回执，再由后台轮询页面状态确认是否进入下一步，从而避免验证码提交触发页面跳转时，旧页面进入 BFCache 导致扩展消息端口被直接关闭。
- Step 4 提交注册验证码后，后台只会在当前标签页**最终稳定停留在 `https://chatgpt.com/` 首页**时，做“已注册账号分支”的早期短路判定；像 `https://auth.openai.com/about-you` 这类新注册资料页，或中途短暂经过的其他跳转页，都会直接放行给原有链路继续判断，不再额外介入。
...
1. 生成随机姓名和生日
2. 后台先检查 Step 4 是否已把当前邮箱判定为“已注册账号分支”；如果是，Step 5 不再下发资料填写命令，而是直接完成当前步骤
3. 若仍处于资料页，内容脚本填写资料；如果资料页出现顶部“我同意以下所有各项”总勾选框，会先自动勾选后再提交
4. 内容脚本点击“完成帐户创建”
5. 点击后立即上报 Step 5 完成，不再等待页面结果
6. 自动运行在进入 Step 6 前仅额外等待当前页面加载完成；如果 Step 5 已被标记为 `skipped`，则会直接进入 Step 6，不再误报失败
...
4. 轮询登录验证码
5. 回填登录验证码
6. 如果登录验证码提交后页面进入 `add-phone / 手机号页`：
   - 默认仍判为 fatal 错误，不再把步骤 8 视为成功
   - 当启用 Hero-SMS 时，允许把它视为“手机号验证分支”成功结束 Step 8，并把后续验证交给 Step 9
...
2. 准备 OAuth 同意页 / 手机号验证码页
3. 如果当前页是 `add-phone`，则先通过 Hero-SMS：
   - 获取或复用 activation
   - 填入手机号
   - 轮询短信验证码
   - 回填短信验证码
4. 继续复用现有 Continue 点击链路：
   - 若仍在验证码页，先点击验证码页 Continue
   - 进入 OAuth 同意页后，再继续点击 OAuth Continue
5. 一旦捕获 localhost callback，写入状态并完成步骤
6. 如果命中 `phone_max_usage_exceeded`，则立即走 fatal 收尾，不再继续当前轮
...
- [outlook-email-utils.js](c:/Users/projectf/Downloads/codex注册扩展/outlook-email-utils.js)
- [background/outlook-email-provider.js](c:/Users/projectf/Downloads/codex注册扩展/background/outlook-email-provider.js)
- [sidepanel/outlook-email-manager.js](c:/Users/projectf/Downloads/codex注册扩展/sidepanel/outlook-email-manager.js)

当前链路：

- sidepanel 中填写 `OutlookEmail Base URL` 与登录密码
- 通过现有 Web 登录态调用 `/login`
- 通过 `/api/groups` 拉取分组并选择“注册邮箱池分组 / 注册成功分组”
- Step 2 前从源分组调用 `/api/accounts?group_id=...` 自动分配当前轮邮箱
- Step 4 / 8 通过 `/api/emails/<email>?folder=all` 轮询验证码
- Step 10 成功后通过 `/api/accounts/batch-update-group` 将当前邮箱移到成功分组，并立即重新拉取 `/api/groups` 刷新 sidepanel 分组数量

边界：

- 当前实现是独立 provider，不影响现有 Hotmail 账号池
- 当前不修改 `references/outlookEmail` 源码，不要求 API Key
- 当前按单实例优先场景设计，通过“成功后移组”完成出池
...
- [hero-sms-utils.js](c:/Users/projectf/Downloads/codex注册扩展/hero-sms-utils.js)
- [background/phone-verification.js](c:/Users/projectf/Downloads/codex注册扩展/background/phone-verification.js)
- [sidepanel/hero-sms-manager.js](c:/Users/projectf/Downloads/codex注册扩展/sidepanel/hero-sms-manager.js)

职责：

- `hero-sms-utils.js`
  - Hero-SMS activation 复用
  - 新手机号获取
  - 短信验证码轮询
  - 本地 `heroSmsPhoneRecords` 去重与 3 次上限控制，以及 `heroSmsExhaustedPhoneRecords` 的本地耗尽记录
  - 如果号码历史已接过验证码，则在本次页面触发发送短信前，把 activation 切到“等待新短信”状态
  - 成功路径不立即 finish activation，号码达到 3 次上限后再完成 activation
  - 按“本次点击发送短信按钮”计时：0 次接码 2 分钟无首码、1~2 次接码 3 分钟无新码时，都会取消当前 activation、标记本地耗尽并抛出专用重试错误
- `background/phone-verification.js`
  - 检测 `add-phone`
  - 在后台直接注入脚本填手机号 / 填验证码
  - 管理运行态 `currentHeroSmsActivationId / currentHeroSmsPhoneNumber / currentHeroSmsActivationStartedAt / currentHeroSmsRequestStartedAt`
  - 在页面真正触发发码动作前，按本地接码历史准备 Hero-SMS 当前 activation 状态
  - 处理 `phone_max_usage_exceeded`、Hero-SMS 验证码超时（首码 / 后续码）与成功/停止后的运行态清理
- `sidepanel/hero-sms-manager.js`
  - 管理启用开关
  - 管理 API Key / 国家代码配置
  - 把 Hero-SMS UI 接线从 `sidepanel/sidepanel.js` 下沉出去
...
5. 执行 `runAutoSequenceFromStep`
   ...
   - 步骤 8 若在验证码提交后进入 `add-phone / 手机号页`，默认仍会抛出 fatal；但启用 Hero-SMS 时，会转入 Step 9 的手机号验证分支
   ...
   - 如果命中 `phone_max_usage_exceeded`，同样不会再做当前轮 retry；会按 skip-failures 策略结束当前轮并决定是否进入下一轮
   - 如果命中 `HERO_SMS_FIRST_CODE_TIMEOUT::*` 或 `HERO_SMS_NEXT_CODE_TIMEOUT::*`，则只放弃**当前 attempt**；自动运行会在同一轮重新打开链路并申请新号码，而不是假设当前页一定支持回退换号
