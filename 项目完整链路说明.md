...
- `hero-sms-utils.js`
  ...
  - 新手机号获取
  - 短信验证码轮询
  - 本地 `heroSmsPhoneRecords` 去重与 3 次上限控制
  - 成功路径不立即 finish activation，号码达到 3 次上限后再完成 activation
  - 若号码在约 125 秒内未收到任何首个验证码，则取消当前 activation、短期隔离该号码并抛出专用重试错误
- `background/phone-verification.js`
  - 检测 `add-phone`
  - 在后台直接注入脚本填手机号 / 填验证码
  - 管理运行态 `currentHeroSmsActivationId / currentHeroSmsPhoneNumber / currentHeroSmsActivationStartedAt`
  - 处理 `phone_max_usage_exceeded`、Hero-SMS 首码超时与成功/停止后的运行态清理
...
5. 执行 `runAutoSequenceFromStep`
   ...
   - 如果命中 `HERO_SMS_FIRST_CODE_TIMEOUT::*`，则只放弃**当前 attempt**；自动运行会在同一轮重新打开链路并申请新号码，而不是假设当前页一定支持回退换号
