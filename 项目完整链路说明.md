...
- 自动重新发送验证码次数现在使用 sidepanel 里的单一“验证码重发”配置；普通邮箱仍按 25 秒间隔节流，Hotmail / 2925 不走这个 25 秒间隔。Step 4 / 8 进入验证码页后默认先轮询当前这次发送产生的验证码，不再在页面刚进入时额外预点一次“重新发送”；只有轮询未命中或页面拒绝验证码时，才会消耗自动重发次数。
