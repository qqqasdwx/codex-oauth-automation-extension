...
- `background/logging-status.js`：后台日志、步骤状态、错误信息和若干状态判断的公共工具层；当前额外承接 `add-phone / 手机号页`、`phone_max_usage_exceeded` 与 `Hero-SMS` 验证码超时（首码 / 后续码）这类认证 / 接码错误的共享判定。
...
- `background/signup-flow-helpers.js`：注册页辅助层，负责打开注册入口、等待密码页以及解析当前流程所用邮箱；当前除 Gmail / 2925 别名邮箱复用外，也会在 `outlookemail-api` 模式下从 OutlookEmail 邮箱池分组中自动分配当前轮邮箱。
...
- `background/verification-flow.js`：注册/登录验证码共享流程层，封装重发、轮询、提交、失败回退、自定义邮箱跳过、共享验证码自动重发次数配置以及 2925 长轮询参数；当前同时接入 Hotmail / LuckMail / OutlookEmail / Cloudflare Temp Email 的后台轮询分支，验证码提交重试上限为 7 次，步骤 8 在默认情况下若提交后进入 `add-phone / 手机号页` 仍会抛出 fatal 错误，但允许在显式开启 `allowAddPhoneSuccess` 时把它转为“手机号验证分支”继续流转，并在 2925 提交成功后异步触发“删除全部邮件”的清理消息。

## `background/steps/`

- `background/steps/clear-login-cookies.js`：步骤 6 实现，负责登录前 Cookie 清理。
- `background/steps/confirm-oauth.js`：步骤 9 实现，负责 localhost 回调监听、在需要时先执行手机号验证、复用验证码页 / OAuth 同意页的 Continue 点击链路，并完成最终 OAuth confirm。
- `background/steps/fetch-login-code.js`：步骤 8 实现，负责登录验证码阶段的邮箱轮询、验证码回填与回退控制；验证码获取后直接提交，不再在提交前回放步骤 7 刷新 OAuth；对非 2925 provider 会固定当前验证码页显示邮箱作为本次 Step 8 的目标邮箱，并在启用 Hero-SMS 时允许把“提交后进入 add-phone”转为 Step 9 的手机号验证分支。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接邮箱记录覆盖层，顶部新增“贡献”按钮并在设置卡片中新增贡献模式主面板；贡献面板内展示 `OAUTH / 回调 / 总状态` 三块真实运行态信息，同时把“本地同步”与“验证码重发”拆成独立行以避免特殊模式隐藏时互相影响；页面继续加载 `managed-alias-utils.js`，并把旧的“邮箱前缀”字段语义改为“别名基邮箱”，同时包含 Hero-SMS 与 OutlookEmail 邮箱池配置区。
- `sidepanel/sidepanel.js`：侧边栏主入口脚本，负责 UI 状态同步、动态步骤渲染、按钮交互、独立本地同步配置、共享验证码自动重发次数配置与广播接收，并装配 Hotmail / OutlookEmail / Hero-SMS / iCloud / LuckMail / 贡献模式 / 邮箱记录面板 manager；当前贡献模式的“开始贡献”会直接复用主自动流程启动逻辑，而独立 manager 负责贡献运行态展示与轮询，同时把 Gmail / 2925 的基邮箱输入、完整注册邮箱输入、自动生成按钮与兼容性校验统一接到共享别名逻辑上。
