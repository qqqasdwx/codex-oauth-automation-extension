...
- `hero-sms-utils.js`：Hero-SMS 纯工具模块，负责 activation 复用、新号获取、短信验证码轮询，以及本地 `heroSmsPhoneRecords` 记录维护。
...
- `background/logging-status.js`：后台日志、步骤状态、错误信息和若干状态判断的公共工具层；当前额外承接 `add-phone / 手机号页`、`phone_max_usage_exceeded` 与 `Hero-SMS` 首码超时这类认证 / 接码错误的共享判定。
...
- `background/phone-verification.js`：手机号验证后台模块，负责检测 `add-phone`、通过 `chrome.scripting.executeScript` 填手机号/短信验证码、管理 Hero-SMS activation 运行态，并在 `phone_max_usage_exceeded`、Hero-SMS 首码超时或号码达到本地 3 次上限时做对应收尾。
...
- `tests/auto-run-phone-max-usage.test.js`：测试自动运行控制器命中 `phone_max_usage_exceeded` 时，会按 skip-failures 策略结束当前轮；命中 `Hero-SMS` 首码超时错误时，则会放弃当前 attempt 并在同一轮重新尝试。
