...
- `.gitignore`：定义仓库忽略规则，当前忽略 `docs/md/`、`.github/`（保留 `workflows/`）、`_metadata/`、`.vscode/` 等目录。
- `LICENSE`：项目许可证文件。
...
- `hero-sms-utils.js`：Hero-SMS 纯工具模块，负责 activation 复用、新号获取、短信验证码轮询，以及本地 `heroSmsPhoneRecords` / `heroSmsExhaustedPhoneRecords` 记录维护；当前会按“本次发码请求时间”执行 0 次接码 2 分钟、1~2 次接码 3 分钟的超时判定，并在号码历史已接过验证码时，于本次页面发码前把 activation 切到等待新短信状态。
...
- `outlook-email-utils.js`：OutlookEmail 邮箱池相关的纯工具函数，负责 Base URL、分组、账号数据归一化，以及运行时账号挑选策略。
...
- `本仓库与上游仓库差别.md`：面向普通使用者的差异说明文档，概括当前 fork 相对上游仓库新增或强化的功能点，不展开代码实现细节。
...
- `.github/workflows/release.yml`：GitHub Actions 发布工作流；在推送 `Pro* / v*` 标签或手动触发时创建/更新 GitHub Release，其中 `Pro*` 标签会额外校验 `manifest.json` 的 `version_name` 与标签一致；同时会构建一个仅包含扩展运行所需文件的发布 zip（排除测试、参考仓库、工作流与开发文档），并作为 release asset 上传。
...
- `background/logging-status.js`：后台日志、步骤状态、错误信息和若干状态判断的公共工具层；当前额外承接 `add-phone / 手机号页`、`phone_max_usage_exceeded`、`user_already_exists` 与 `Hero-SMS` 验证码超时（首码 / 后续码）这类认证 / 接码错误的共享判定。
...
- `background/outlook-email-provider.js`：OutlookEmail provider 领域模块，负责登录 Web API、拉取分组/账号/邮件、轮询验证码，以及注册成功后的移组收尾。
- `background/panel-bridge.js`：来源桥接层；CPA / SUB2API 继续封装页面打开、脚本注入和通信，Codex2API 则直接通过后台协议生成 OAuth 地址。
- `background/phone-verification.js`：手机号验证后台模块，负责检测 `add-phone`、通过 `chrome.scripting.executeScript` 填手机号/短信验证码、管理 Hero-SMS activation / 发码请求运行态（含 `currentHeroSmsRequestStartedAt`），并在真正触发页面发码前按接码历史准备 Hero-SMS 状态；同时在 `phone_max_usage_exceeded`、Hero-SMS 验证码超时或号码达到本地 3 次上限时做对应收尾。
- `background/signup-flow-helpers.js`：注册页辅助层，负责打开注册入口、等待密码页以及解析当前流程所用邮箱；在 Gmail 与 `2925 + provide` 模式下会优先复用已经存在且仍兼容的完整注册邮箱，只有不兼容或为空时才重新生成；当前在 `outlookemail-api` 模式下，也会从 OutlookEmail 邮箱池分组中自动分配当前轮邮箱；当 provider 为 2925 且启用了 provide 模式下的号池时，会先确保账号池中已选中可用账号。
...
- `background/verification-flow.js`：注册/登录验证码共享流程层，封装重发、轮询、提交、失败回退、自定义邮箱跳过、共享验证码自动重发次数配置以及 2925 长轮询参数；当前同时接入 Hotmail / LuckMail / OutlookEmail / Cloudflare Temp Email 的后台轮询分支，验证码提交重试上限为 15 次，2925 每次重发验证码之间都会固定跑完一轮 15 次邮箱刷新轮询；若 `mail2925Mode = receive`，会额外把目标注册邮箱传给 2925 内容脚本做弱匹配；若 2925 轮询命中“子邮箱已达上限邮箱”，会转入 2925 会话模块执行“记录时间、禁用 24 小时、切下一个账号并重新登录”，然后直接结束当前尝试；步骤 8 在默认情况下若提交后进入 `add-phone / 手机号页` 仍会抛出 fatal 错误，但允许在显式开启 `allowAddPhoneSuccess` 时把它转为“手机号验证分支”继续流转，并在 2925 提交成功后异步触发“删除全部邮件”的清理消息。

## `background/steps/`

- `background/steps/clear-login-cookies.js`：步骤 6 实现，负责登录前 Cookie 清理。
- `background/steps/confirm-oauth.js`：步骤 9 实现，负责 OAuth 同意页按钮定位、点击、localhost 回调监听、在需要时先执行手机号验证，并完成最终 OAuth confirm。
- `background/steps/fetch-login-code.js`：步骤 8 实现，负责登录验证码阶段的邮箱轮询、验证码回填与回退控制；验证码获取后直接提交，不再在提交前回放步骤 7 刷新 OAuth；对非 2925 provider 会固定当前验证码页显示邮箱作为本次 Step 8 的目标邮箱；当 provider 为 2925 时，会在轮询前先确保当前 2925 账号已自动登录；启用 Hero-SMS 时允许把“提交后进入 add-phone”转为 Step 9 的手机号验证分支。
...
- `content/signup-page.js`：注册、登录、授权主内容脚本，负责 OpenAI / ChatGPT 页面上的步骤执行；其中 Step 2 / 3 的延迟点击与延迟提交在真正触发前会先检查 Stop 状态，避免停止后页面继续自动点击；当前 Step 9 的 Continue 查找 / 点击逻辑已最小扩展到“OAuth 同意页 + 手机号短信验证码页”双场景复用。
...
- `docs/refactor/2026-04-19-outlookemail-provider-tasklist.md`：OutlookEmail 邮箱池 provider 接入的独立实施清单、验收标准与回归跟踪文档。
- `docs/refactor/2026-04-19-phone-verification-merge-tasklist.md`：Hero-SMS / 手机号验证能力接入的实施清单、验收标准与测试清单。
...
- `sidepanel/hero-sms-manager.js`：侧边栏 Hero-SMS 管理器，负责启用/禁用切换、API Key 显隐、国家代码跳转与最小 collect/apply/bind 接线。
...
- `sidepanel/outlook-email-manager.js`：侧边栏 OutlookEmail 管理器，负责 Base URL / 登录密码输入、分组拉取按钮、分组下拉渲染与最小自动保存接线。
...
- `sidepanel/sidepanel.html`：侧边栏页面结构；当前步骤列表已改为动态容器，日志区提供“记录”按钮并挂接邮箱记录覆盖层，顶部新增“贡献”按钮并在设置卡片中新增贡献模式主面板；贡献按钮下方额外挂接一个可关闭的轻提示气泡，用于提示公开公告 / 使用教程有更新；贡献面板内展示 `OAUTH / 回调 / 总状态` 三块真实运行态信息，同时把“本地同步”与“验证码重发”拆成独立行以避免特殊模式隐藏时互相影响；页面继续加载 `managed-alias-utils.js`，并把旧的“邮箱前缀”字段语义改为“别名基邮箱”，同时包含 Hero-SMS、OutlookEmail 邮箱池与 2925 账号池配置区；当 provider 为 2925 时，会额外显示 `提供邮箱 / 接收邮箱` 模式切换，并把“2925 号池”从别名基邮箱行拆成独立配置行，避免 receive 模式把账号池开关一起隐藏；来源下拉框当前支持 `CPA / SUB2API / Codex2API`，其中 Codex2API 额外提供后台地址和管理密钥配置行；Hotmail / 2925 两个账号池当前都使用统一的头部“添加账号/取消添加”按钮和共享表单容器。
- `sidepanel/sidepanel.js`：侧边栏主入口脚本，负责 UI 状态同步、动态步骤渲染、按钮交互、独立本地同步配置、共享验证码自动重发次数配置与广播接收，并装配 Hotmail / OutlookEmail / 2925 / Hero-SMS / iCloud / LuckMail / 贡献模式 / 邮箱记录面板 / 贡献内容更新服务；当前贡献模式的“开始贡献”会直接复用主自动流程启动逻辑，而独立 manager 负责贡献运行态展示与轮询，同时把 Gmail / 2925 的基邮箱输入、完整注册邮箱输入、自动生成按钮与兼容性校验统一接到共享别名逻辑上；当 provider 为 2925 时，会根据 `mail2925Mode` 决定是否启用别名基邮箱链路，而 2925 号池开关与当前账号选择则独立显示并同时服务于 provide / receive 两种模式；新来源 Codex2API 在这里仅补充来源配置接线、表单显隐和 Step 10 按钮文案，不承接协议业务逻辑；Hotmail / 2925 的新增表单显隐统一接到 `sidepanel/account-pool-ui.js` 这一层共享 helper；侧边栏初始化与点击“自动”前会刷新一次贡献站公开内容摘要，并按本地关闭版本决定是否展示轻提示。
...
- `tests/auto-run-phone-max-usage.test.js`：测试自动运行控制器命中 `phone_max_usage_exceeded` 时，会按 skip-failures 策略结束当前轮；命中 `Hero-SMS` 验证码超时错误时，则会放弃当前 attempt 并在同一轮重新尝试。
...
- `tests/auto-run-step4-restart.test.js`：测试自动运行在步骤 4 失败时会沿用当前邮箱回到步骤 1 重开，并在步骤 4 已判定“跳过步骤 5”时直接续跑步骤 6。
...
- `tests/background-message-router-step2-skip.test.js`：测试步骤 2 / 4 直接落到后续链路时的跳步与状态保护逻辑，包括步骤 4 命中“已注册账号”分支后自动跳过步骤 5。
...
- `tests/background-phone-verification-module.test.js`：测试手机号验证后台模块的导出、`add-phone` 检测与 Hero-SMS 接线。
...
- `tests/background-step1-cookie-cleanup.test.js`：测试步骤 1 会先清理 cookies、关闭旧的注册相关标签页，再重新打开 ChatGPT 官网。
- `tests/background-step5-submit-short-circuit.test.js`：测试步骤 5 会把生成好的资料直接转发给内容脚本，并在步骤 4 已明确标记跳过资料页时直接短路完成当前步骤。
- `tests/background-step4-verification-flow.test.js`：测试步骤 4 进入注册验证码页后会先轮询当前验证码，而不是预先自动重发。
...
- `tests/hero-sms-utils.test.js`：测试 Hero-SMS 工具层的 activation 复用、手机号验证码去重与上限控制。
...
- `tests/outlook-email-provider.test.js`：测试 OutlookEmail provider 模块的登录态建立、分组拉取与运行时状态同步。
- `tests/outlook-email-utils.test.js`：测试 OutlookEmail 工具层的 Base URL、分组/账号归一化与账号挑选策略。
...
- `tests/sidepanel-hero-sms-manager.test.js`：测试侧边栏 Hero-SMS 管理器的 collect/apply/bind 接线、显隐和跳转行为。
...
- `tests/sidepanel-outlook-email-manager.test.js`：测试侧边栏 OutlookEmail 管理器脚本加载顺序、工厂导出与分组渲染。
...
- `tests/step9-phone-verification.test.js`：测试步骤 9 在手机号验证分支下，会先完成 Hero-SMS 手机号验证，再继续复用 Continue / localhost 回调监听链路。
...
- `tests/verification-flow-polling.test.js`：测试 2925 长轮询参数、验证码提交流程中的 `beforeSubmit` 钩子执行顺序、验证码提交改为“先回执再轮询结果”的链路，以及步骤 4 仅在标签页最终稳定停留于 `https://chatgpt.com/` 首页时才命中“已注册账号”分支、步骤 8 提交验证码后在默认模式下进入手机号页会报 fatal、在 Hero-SMS 模式下可转入手机号验证分支。
